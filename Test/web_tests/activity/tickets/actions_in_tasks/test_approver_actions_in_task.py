...
@allure.epic('G2')
@allure.feature('Web - Approver actions in task')
@allure.story('Активности согласующего в задаче')
class TestApproverActionsInTask(WebBase):
    ...
    @allure.title('Отклонить задачу в Ленте Активности')
    @pytest.mark.WebTest
    @allure.description('Белопросов Максим')
    def test_reject_task_from_activity(self):
    ...
    @allure.title('Согласовать задачу в Ленте Активности')
    @pytest.mark.WebTest
    @allure.description('Белопросов Максим')
    def test_accept_task_from_activity(self):
        # Создаем задачу с согласующим
        approver = 'test_user05'
        task = self.create_task({"contractorId": 'test_user01', "approvers": [approver], "descriptionContent": ["TestDescription AutomationWebTests " + self.APP.time.get_date_time_Y_m_d_H_M_S()]})
        # Пишем комментарий, чтобы тикет отобразился в Ленте Активности
    ...
    @allure.title('Делегировать согласование в задаче в Ленте Активности')
    @pytest.mark.WebTest
    @allure.description('Белопросов Максим')
    def test_delegate_agreement_in_task_from_activity(self):
        # Создаем задачу с согласующим
    ...
    @allure.title('Задать уточнение инициатору в задаче из Ленты Активности')
    @pytest.mark.WebTest
    @allure.description('Белопросов Максим')
    def test_ask_initiator_clarification_in_task_from_activity(self):
        # Создаем задачу с согласующим
        approver = 'test_user05'
        task = self.create_task({"contractorId": 'test_user01', "approvers": [approver], "descriptionContent": ["TestDescription AutomationWebTests " + self.APP.time.get_date_time_Y_m_d_H_M_S()]})
        # Пишем комментарий, чтобы тикет отобразился в Ленте Активности
    ...
    @allure.title('Задать уточнение исполнителю в задаче из Ленты Активности')
    @pytest.mark.WebTest
    @allure.description('Белопросов Максим')
    def test_ask_contractor_clarification_in_task_from_activity(self):
        # Создаем задачу с согласующим
        approver = 'test_user05'
        contractor = 'test_user01'
        task = self.create_task({"contractorId": contractor, "approvers": [approver], "descriptionContent": ["TestDescription AutomationWebTests " + self.APP.time.get_date_time_Y_m_d_H_M_S()]})
