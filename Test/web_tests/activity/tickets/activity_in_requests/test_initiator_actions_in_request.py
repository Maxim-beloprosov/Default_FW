import allure
import pytest

from Test.web_tests.web_base import WebBase

@allure.epic('G2')
@allure.feature('Web - Initiator actions in request')
@allure.story('Активности инициатора в заявке')
class TestInitiatorActionsInRequest(WebBase):

    def setup_method(self):
        self.APP.web_any_page.open_main_page()
        self.APP.api_token.get_token('test_user09')

    @allure.title('Отменить заявку из Ленты Активности')
    @pytest.mark.WebTest
    @allure.description('Белопросов Максим')
    def test_cancel_request_from_activity(self):
        # Создаем заявку
        request = self.create_request({"descriptionContent": ["TestDescription AutomationWebTests " + self.APP.time.get_date_time_Y_m_d_H_M_S()]})
        # Авторизуемся участником ГО
        self.APP.api_token.get_token('test_user01')
        # Пишем комментарий, чтобы заявка отобразилась в Ленте Активности
        self.APP.api_actions_in_comment.create_comment(request['id'])
        # Выносим тип и номер тикета
        type_and_number_a_ticket = [request['ticketType'], request['docNumber']]
        # Авторизуемся инициатором
        self.APP.web_any_page.check_autorizated_user_and_loging_other_user_if_need(self.users['test_user09'])
        # Отменяем заявку
        correct_status = self.APP.web_activity.canceling_of_ticket(type_and_number_a_ticket)
        # Получаем актуальный статус тикета из Ленты Активности
        actual_status = self.APP.web_activity.get_status_ticket_from_activity(type_and_number_a_ticket[1])
        # Сравниваем 2 статуса
        assert correct_status == actual_status

    test_data = [1, 2, 3, 4, 5]
    @allure.title('Закрыть заявку из Ленты Активности')
    @pytest.mark.WebTest
    @allure.description('Белопросов Максим')
    @pytest.mark.parametrize('rating', test_data)
    def test_closed_request_from_activity(self, rating):
        # Создаем заявку
        request = self.create_request({"descriptionContent": ["TestDescription AutomationWebTests " + self.APP.time.get_date_time_Y_m_d_H_M_S()]})
        # Авторизуемся участником ГО
        self.APP.api_token.get_token('test_user01')
        # Пишем комментарий, чтобы заявка отобразилась в Ленте Активности
        self.APP.api_actions_in_comment.create_comment(request['id'])
        # Назначаем заявку на себя
        request = self.APP.api_actions_in_request.assign_to_me(request['syncToken'], request['id'])
        # Переводим заявку в проверку
        request = self.APP.api_actions_in_request.send_request_to_resolved(request['syncToken'], request['id'])
        # Авторизуемся инициатором
        self.APP.web_any_page.check_autorizated_user_and_loging_other_user_if_need(self.users['test_user09'])
        # Выносим тип и номер тикета
        type_and_number_a_ticket = [request['ticketType'], request['docNumber']]
        # Закрываем заявку с оценкой в 1-5 баллов
        correct_status = self.APP.web_activity.ticket_to_closed_from_activity(type_and_number_a_ticket, rating)
        # Получаем актуальный статус заявки
        actual_status = self.APP.web_activity.get_status_ticket_from_activity(type_and_number_a_ticket[1])
        # Сравниваем 2 статуса заявки
        assert correct_status == actual_status

    @allure.title('Апеллировать заявку из Ленты Активности')
    @pytest.mark.WebTest
    @allure.description('Белопросов Максим')
    def test_appeal_request_from_activity(self):
        # Создаем заявку с согласующим
        approver = 'test_user05'
        request = self.create_request({"approvers": [approver], "descriptionContent": ["TestDescription AutomationWebTests " + self.APP.time.get_date_time_Y_m_d_H_M_S()]})
        # Пишем комментарий, чтобы заявка отобразилась в Ленте Активности
        self.APP.api_actions_in_comment.create_comment(request['id'])
        # Выносим тип и номер тикета
        type_and_number_a_ticket = [request['ticketType'], request['docNumber']]
        # Авторизуемся согласующим
        self.APP.api_token.get_token(approver)
        # Принимаем отрицательное решение
        self.APP.api_actions_in_request.reject_request(request['syncToken'], request['id'])
        # Авторизуемся инициатором
        self.APP.web_any_page.check_autorizated_user_and_loging_other_user_if_need(self.users['test_user09'])
        # Апеллируем заявку
        correct_status = self.APP.web_activity.appeal_a_ticket_from_activity(type_and_number_a_ticket)
        # Получаем актуальный статус заявки
        actual_status = self.APP.web_activity.get_status_ticket_from_activity(type_and_number_a_ticket[1])
        # Переходим в тикет с нужным номером
        self.APP.web_activity.click_number_with_need_ticket(type_and_number_a_ticket[1])
        # Получаем информацию о всех согласующих
        list_approvers = self.APP.web_tickets_base.get_approvers_in_ticket()
        # Формируем ОР согласующего (Руководителя согласующего, который принял отрицательное решение)
        correct_approver = self.users[approver]['Manager']
        correct_approver = self.users[correct_approver]['Surname'] + ' ' + self.users[correct_approver]['Name']
        # Сравниваем 2 статуса
        assert correct_status == actual_status
        # Сравниваем ожидаемого согласующего с фактическим
        assert correct_approver in list_approvers

    @allure.title('Назначить заявку на себя из Ленты Активности')
    @pytest.mark.WebTest
    @allure.description('Белопросов Максим')
    def test_assign_request_to_me_from_activity(self):
        # Авторизуемся участником ГО
        initiator = 'test_user01'
        self.APP.api_token.get_token(initiator)
        # Создаем заявку
        request = self.create_request({"descriptionContent": ["TestDescription AutomationWebTests " + self.APP.time.get_date_time_Y_m_d_H_M_S()]})
        # Авторизуемся другим участником ГО
        self.APP.api_token.get_token('test_user04')
        # Пишем комментарий, чтобы заявка отобразилась в Ленте Активности
        self.APP.api_actions_in_comment.create_comment(request['id'])
        # Выносим тип и номер тикета
        type_and_number_a_ticket = [request['ticketType'], request['docNumber']]
        # Авторизуемся инициатором
        self.APP.web_any_page.check_autorizated_user_and_loging_other_user_if_need(self.users[initiator])
        # Назначаем заявку на себя
        correct_contractor = self.APP.group_data.users['test_user01']['Surname'] + ' ' + self.APP.group_data.users['test_user01']['Name']
        self.APP.web_activity.assign_request_to_me_from_activity(type_and_number_a_ticket, correct_contractor)
        # Переходим в тикет с нужным номером
        self.APP.web_activity.click_number_with_need_ticket(type_and_number_a_ticket[1])
        # Получаем исполнителя заявки
        actual_contractor = self.APP.web_tickets_base.get_contractor_ticket()
        # Сравниваем 2 статуса
        assert correct_contractor == actual_contractor

    @allure.title('Отправить заявку на доработку из Ленты Активности')
    @pytest.mark.WebTest
    @allure.description('Белопросов Максим')
    def test_send_request_for_overwork_from_activity(self):
        # Создаем заявку
        request = self.create_request({"descriptionContent": ["TestDescription AutomationWebTests " + self.APP.time.get_date_time_Y_m_d_H_M_S()]})
        # Авторизуемся участником ГО
        self.APP.api_token.get_token('test_user01')
        # Пишем комментарий, чтобы заявка отобразилась в Ленте Активности
        self.APP.api_actions_in_comment.create_comment(request['id'])
        # Назначаем заявку на себя
        request = self.APP.api_actions_in_request.assign_to_me(request['syncToken'], request['id'])
        # Переводим заявку в проверку
        request = self.APP.api_actions_in_request.send_request_to_resolved(request['syncToken'], request['id'])
        # Выносим тип и номер тикета
        type_and_number_a_ticket = [request['ticketType'], request['docNumber']]
        # Авторизуемся инициатором
        self.APP.web_any_page.check_autorizated_user_and_loging_other_user_if_need(self.users['test_user09'])
        # Возвращаем заявку на доработку
        correct_status = self.APP.web_activity.ticket_to_overwork_from_activity(type_and_number_a_ticket)
        # Получаем актуальный статус заявки
        actual_status = self.APP.web_activity.get_status_ticket_from_activity(type_and_number_a_ticket[1])
        # Сравниваем 2 статуса заявки
        assert correct_status == actual_status
